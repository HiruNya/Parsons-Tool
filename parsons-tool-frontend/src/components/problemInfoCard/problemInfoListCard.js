    ...
    return (
        <>
          <div className="bg-sky-200 m-5 w-9/10 p-2 rounded-xl flex flex-row items-center" onClick={()=> setSelected(problem)}>
            ...
            <button className="px-3 py-1 ml-auto mr-1 w-min h-min border-2 border-black bg-transparant rounded-full text-xl font-medium hover:text-white hover:border-white" onClick={() => setSelected(problem)}> &gt; </button>
          </div>
        </>
      );
}